...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            second_module = model.model[1]
            if first_module.__class__.__name__ == 'Identity' and second_module.__class__.__name__ != 'InputSel_m3data':
                im = torch.empty((1, 9, *imgsz), device=p.device, dtype=p.dtype)  # input image in BCHW format
            elif first_module.__class__.__name__ == 'Identity' and second_module.__class__.__name__ == 'InputSel_m3data':
                im = torch.empty((1, 6, *imgsz), device=p.device, dtype=p.dtype)
            else:
