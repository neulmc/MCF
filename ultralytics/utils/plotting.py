    ...
    def __init__(
        ...
        if not input_is_pil:
            if im.shape[2] == 1:  # handle grayscale
                ...
                im = np.ascontiguousarray(im[..., :3])  #只保留前3通道
                #im = np.ascontiguousarray(im[..., 6:9])  # ir
                #im = np.ascontiguousarray(im[..., 3:6])  # depth
        if self.pil:  # use PIL
...
def plot_images(
    ...
    if c == 2:
        ...
        #images = images[:, 3:6]  # depth lmc
        #images = images[:, 6:9]  # depth lmc
