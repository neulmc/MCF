...
class MultiModalYOLODataset(YOLODataset):
    ...
    假设目录结构:
    root/
      images/          # RGB图 (必须叫images，因为img2label_paths硬编码)
      infrared/        # 红外图
      depth/           # 深度图
      labels/          # 标签
    """

    def __init__(self, *args, modal_paths: dict = None, **kwargs):
        """
        Args:
            modal_paths: 其他模态的路径配置，例如:
                {
                    'infrared': 'infrared',   # 相对于root的路径
                    'depth': 'depth'
                }
        """
        self.modal_paths = modal_paths or {}
        # channels = 9 lmc lmc lmc
    ...
    def load_image(
        ...
        im_depth_ir, f, fn = self.ims[i], self.im_files[i], self.npy_files[i]
        if im_depth_ir is None:  # not cached in RAM
            if fn.exists():  # load npy
                try:
                    im_depth_ir = np.load(fn)
                    npy_channels = im_depth_ir.shape[-1] if im_depth_ir.ndim >= 3 else 1
                    if npy_channels != self.channels:
                        ...
                        depth = imread(f.replace('images', 'depth'), flags=self.cv2_flag)
                        ir = imread(f.replace('images', 'infrared'), flags=self.cv2_flag)
            ...
            if im.ndim == 2:
                ...
                depth = depth[..., None]
                ir = ir[..., None]
            im_depth_ir = np.concatenate([im, depth, ir], axis=-1)
            # Add to buffer if training with augmentations
            if self.augment:
                self.ims[i], self.im_hw0[i], self.im_hw[i] = im_depth_ir, (h0, w0), im_depth_ir.shape[:2]  # im, hw_original, hw_resized
            ...
            return im_depth_ir, (h0, w0), im_depth_ir.shape[:2]

        return self.ims[i], self.im_hw0[i], self.im_hw[i]

class MultiModalYOLODataset_m3data(YOLODataset):
    """
    支持多模态输入的YOLO数据集
    假设目录结构:
    root/
    ...
    def load_image(
        ...
        im_ir, f, fn = self.ims[i], self.im_files[i], self.npy_files[i]
        if im_ir is None:  # not cached in RAM
            if fn.exists():  # load npy
                try:
                    im_ir = np.load(fn)
                    npy_channels = im_ir.shape[-1] if im_ir.ndim >= 3 else 1
                    if npy_channels != self.channels:
                        LOGGER.warning(
                            f"{self.prefix}Removing stale *.npy image file {fn} with {npy_channels} channels, expected {self.channels}"
                        )
            ...
            if im.ndim == 2:
                im = im[..., None]
                ir = ir[..., None]
            im_ir = np.concatenate([im, ir], axis=-1)
            # Add to buffer if training with augmentations
            if self.augment:
                self.ims[i], self.im_hw0[i], self.im_hw[i] = im_ir, (h0, w0), im_ir.shape[:2]  # im, hw_original, hw_resized
                self.buffer.append(i)
                if 1 < len(self.buffer) >= self.max_buffer_length:  # prevent empty buffer
                    j = self.buffer.pop(0)
                    if self.cache != "ram":
            ...
            return im_ir, (h0, w0), im_ir.shape[:2]
