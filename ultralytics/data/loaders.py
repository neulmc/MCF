...
class LoadImagesAndVideos:
    ...
    def __init__(self, path: str | Path | list, batch: int = 1, vid_stride: int = 1, channels: int = 3, modal=None):
    ...
    def __next__(self) -> tuple[list[str], list[np.ndarray], list[str]]:
        ...
        while len(imgs) < self.bs:
            ...
            if self.video_flag[self.count]:
                ...
                depth0 = imread(path.replace('images','depth'), flags=self.cv2_flag)  # BGR
                ir0 = imread(path.replace('images','infrared'), flags=self.cv2_flag)  # BGR
                if im0 is None:
                    LOGGER.warning(f"Image Read Error {path}")
                else:
                    paths.append(path)
                    imgs.append(np.concatenate([im0, depth0, ir0], axis=2))
                ...
                im0 = imread(path, flags=self.cv2_flag)  # BGR
                if 'ir' in self.modal:
                   im01 = imread(path.replace('images','infrared'), flags=self.cv2_flag)  # BGR
                elif 'depth' in self.modal:
                    im01 = imread(path.replace('images', 'depth'), flags=self.cv2_flag)  # BGR
                if im0 is None:
                    LOGGER.warning(f"Image Read Error {path}")
                else:
                    paths.append(path)
                    imgs.append(np.concatenate([im0, im01], axis=2))
                    info.append(f"image {self.count + 1}/{self.nf} {path}: ")
                self.count += 1  # move to the next file
                if self.count >= self.ni:  # end of image list
                    break
