...
class RandomHSV(BaseTransform):
    ...
    def apply_image(self, labels, params: dict[str, Any] | None = None):
        ...
        img = labels["img"]
        if img.shape[-1] != 3:  # 9 通道
            # 取前 3 通道，强制拷贝为连续数组
            img_rgb = img[:, :, :3].copy()  # 👈 加 .copy()
            img_rest = img[:, :, 3:]  # depth + ir 保持不变
            ...
            if self.hgain or self.sgain or self.vgain:
                ...
                lut_sat[0] = 0
                hue, sat, val = cv2.split(cv2.cvtColor(img_rgb, cv2.COLOR_BGR2HSV))
                im_hsv = cv2.merge((cv2.LUT(hue, lut_hue), cv2.LUT(sat, lut_sat), cv2.LUT(val, lut_val)))
                cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR, dst=img_rgb)  # img_rgb 现在是连续数组
            # 拼接回去
