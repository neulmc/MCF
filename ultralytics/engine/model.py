...
class Model(torch.nn.Module):
    ...
    def load_proj(self, weights: str | Path = "yolo26n.pt", idx_mapping= dict()) -> Model:
        """Load parameters from the specified weights file into the model.
        ...
        if isinstance(weights, (str, Path)):
            self.overrides["pretrained"] = weights  # remember the weights for DDP training
            weights, self.ckpt = load_checkpoint(weights)
        self.model.load_proj(weights, idx_mapping)
        return self
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        # 获取当前执行的主脚本路径
        main_script = sys.argv[0]  # 比如 python train.py → train.py 的完整路径
        if os.path.isfile(main_script):
            dst = os.path.join(self.trainer.save_dir, os.path.basename(main_script))
