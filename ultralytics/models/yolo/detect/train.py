...
class DetectionTrainer(BaseTrainer):
    ...
    def build_dataset(self, img_path: str, mode: str = "train", batch: int | None = None,
                      lmc_multi_modal = False, lmc_multi_modal_m3data = False):
        ...
        return build_yolo_dataset(self.args, img_path, batch, self.data, mode=mode, rect=mode == "val", stride=gs,
                                  lmc_multi_modal=lmc_multi_modal, lmc_multi_modal_m3data = lmc_multi_modal_m3data)
    ...
    def plot_training_labels(self):
        ...
        val_labels = self.test_loader.dataset.labels
        combined_boxes = np.concatenate([lb["bboxes"] for lb in train_labels] + [lb["bboxes"] for lb in val_labels], 0)
        combined_cls = np.concatenate([lb["cls"] for lb in train_labels] + [lb["cls"] for lb in val_labels],
                                      0).squeeze()
        np.savez('E:/ultralytics/plot_lmc' + "/combined_labels.npz", boxes=combined_boxes, cls=combined_cls, names=self.data["names"])
