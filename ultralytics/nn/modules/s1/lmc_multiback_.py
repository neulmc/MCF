...
class TriMFBlockback(nn.Module):
    """三模态：RGB + Depth + IR"""
...
class PConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # 普通卷积
        #out_cv = self.conv1(x_fp32, weight_fp32, bias_fp32, self.stride, self.padding, self.dilation, self.groups)
        out_cv = F.conv2d(x, self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
        # 三种差分 (共享 weight)
...
class RDConv2d(nn.Module):
    ...
    def forward(self, x, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
        ...
        buffer = torch.zeros(shape[0], shape[1], 5 * 5, device=weight.device, dtype=weight.dtype)  # 逐步改
        w = weight.view(shape[0], shape[1], -1)
...
class lmcTripleDEAback(nn.Module):
    """三模态 DEA：RGB + Depth + IR"""

    def __init__(self, channel=(512, 512, 512), kernel_size=80, p_kernel=None, m_kernel=None, reduction=16):
        super().__init__()
        self.deca = GridChannelQKVFusion(channel, head_dim=16)
        self.depa = lmcTripleDEPAback(channel, m_kernel)

        c_rgb, c_depth, c_ir = channel[0], channel[1], channel[2]
        self.c_depth = c_depth
        self.c_ir = c_ir

        # 三模态 concat 后投影回原通道（保持输出通道数不变）
        self.proj = Conv(c_rgb + c_depth + c_ir, c_rgb, 1,)
...
class GridChannelQKVFusion(nn.Module):
    """
    空间网格 + 通道 Q/K/V 三模态融合
    把特征图分成 8×8 网格，每个网格做通道级注意力
    """
...
class lmcTripleDEPAback(nn.Module):
    """RGB + Depth + IR 三模态空间 融合"""
    def __init__(self, channel=512, m_kernel=None):
        super().__init__()
        if m_kernel is None:
