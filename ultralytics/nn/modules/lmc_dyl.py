...
class DEA3(nn.Module):
    ...
    def __init__(self, channel=512, kernel_size=80, p_kernel=None, m_kernel=None, reduction=16):
        super().__init__()
        self.deca = DECA3(channel, kernel_size, p_kernel, reduction)
        self.depa = DEPA3(channel, m_kernel)
        self.act = nn.Sigmoid()

    def forward(self, x):
        result_vi, result_depth, result_ir = self.depa(self.deca(x))
        return self.act(result_vi + result_depth + result_ir)


class DECA3(nn.Module):
    """x0 --> RGB feature map,  x1 --> IR feature map"""
    ...
    def forward(self, x):
        b, c, h, w = x[0].size()
        w_vi = self.avg_pool(x[0]).view(b, c)
        w_depth = self.avg_pool(x[1]).view(b, c)
        w_ir = self.avg_pool(x[2]).view(b, c)
        w_vi = self.fc(w_vi).view(b, c, 1, 1)
        w_depth = self.fc(w_depth).view(b, c, 1, 1)
        w_ir = self.fc(w_ir).view(b, c, 1, 1)

        glob_t = self.compress(torch.cat([x[0], x[1], x[2]], 1))
        glob = self.conv_c3(self.conv_c2(self.conv_c1(glob_t))) if min(h, w) >= self.kernel_size else torch.mean(
                                                                                    glob_t, dim=[2, 3], keepdim=True)
        result_vi = x[0] * (self.act(w_depth * glob + w_ir * glob)).expand_as(x[0])
        result_depth = x[1] * (self.act(w_vi * glob + w_ir * glob)).expand_as(x[1])
        result_ir = x[2] * (self.act(w_vi * glob + w_depth * glob)).expand_as(x[2])

        return result_vi, result_depth, result_ir


class DEPA3(nn.Module):
    """x0 --> RGB feature map,  x1 --> IR feature map"""
    def __init__(self, channel=512, m_kernel=None):
        super().__init__()
        self.conv1 = Conv(2, 1, 5)
        self.conv2 = Conv(2, 1, 5)
        self.conv3 = Conv(2, 1, 5)
        self.compress1 = Conv(channel, 1, 3)
        self.compress2 = Conv(channel, 1, 3)
        self.compress3 = Conv(channel, 1, 3)
        self.act = nn.Sigmoid()
        ...
        self.cv_v1 = Conv(channel, 1, m_kernel[0])
        self.cv_v2 = Conv(channel, 1, m_kernel[1])
        self.cv_d1 = Conv(channel, 1, m_kernel[0])
        self.cv_d2 = Conv(channel, 1, m_kernel[1])
        self.cv_i1 = Conv(channel, 1, m_kernel[0])
        self.cv_i2 = Conv(channel, 1, m_kernel[1])

    def forward(self, x):
        w_vi = self.conv1(torch.cat([self.cv_v1(x[0]), self.cv_v2(x[0])], 1))
        w_de = self.conv2(torch.cat([self.cv_d1(x[1]), self.cv_d2(x[1])], 1))
        w_ir = self.conv3(torch.cat([self.cv_i1(x[2]), self.cv_i2(x[2])], 1))
        glob = self.act(self.compress1(x[0]) + self.compress2(x[1]) + self.compress3(x[2]))
        w_vi = self.act(glob + w_vi)
        w_de = self.act(glob + w_de)
        w_ir = self.act(glob + w_ir)
        result_vi = x[0] * w_de.expand_as(x[0]) + x[0] * w_ir.expand_as(x[0])
        result_de = x[1] * w_vi.expand_as(x[1]) + x[1] * w_ir.expand_as(x[1])
        result_ir = x[2] * w_vi.expand_as(x[2]) + x[2] * w_de.expand_as(x[2])

        return result_vi, result_de, result_ir
