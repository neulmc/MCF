...
class BaseModel(torch.nn.Module):
    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            ...
            try:
                n_l, n_p, n_g, flops = self.info(verbose=verbose)
                self.n_l = n_l
                self.n_p = n_p
                self.n_g = n_g
                self.flops = flops
    ...
    '''
    def load_proj(self, weights, insert_pos, verbose=True):
        model = weights["model"] if isinstance(weights, dict) else weights  # torchvision models are not dicts
        csd = model.float().state_dict()  # checkpoint state_dict as FP32
        ...
        csd_new = {}
        proj_name = {}
        for old_name in csd.keys():
            parts = old_name.split('.')
            idx = int(parts[1])
            shift = sum(1 for pos in insert_pos if pos <= idx)
            new_idx = idx + shift
            new_name = f"model.{new_idx}." + '.'.join(parts[2:])
            proj_name[new_name] = old_name
        for name, param in my_state.items():
            name_ori = proj_name[name]
            if name_ori in csd and csd[name_ori].shape == param.shape:
                csd_new[name] = csd[name_ori]  # 匹配上了，用预训练值
    ...
    def load_proj(self, weights, idx_mapping, verbose=True):
        model = weights["model"] if isinstance(weights, dict) else weights  # torchvision models are not dicts
        csd = model.float().state_dict()  # checkpoint state_dict as FP32
        ...
        for new_name in my_state.keys():
            parts = new_name.split('.')
            idx = int(parts[1])
            old_idx = idx_mapping.get(idx)  # 新索引→旧索引
            if old_idx is None:
                continue
            old_name = f"model.{old_idx}." + '.'.join(parts[2:])
            proj_name[new_name] = old_name  # key=新，value=旧

        for name, param in my_state.items():
            name_ori = proj_name.get(name)
            if name_ori is not None and name_ori in csd and csd[name_ori].shape == param.shape:
                csd_new[name] = csd[name_ori].clone()
            #if name_ori is not None and name_ori in csd:
            #    if csd[name_ori].shape == param.shape:
            #        csd_new[name] = csd[name_ori].clone()
            #    else:
            #        slices = tuple(slice(0, min(ts, ss)) for ts, ss in zip(param.shape, csd[name_ori].shape))
            #        csd_new[name] = csd[name_ori][slices].clone()
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo26n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            #m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s))])  # forward old
            if isinstance(cfg, dict) and cfg.get('lmc_input') is not None:
                m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, cfg['lmc_input'], s, s))])
            else:
                m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s))])
            #m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, cfg.channel, s, s))])  # forward lmc lmc
...
def parse_model(d, ch, verbose=True):
    ...
    lmc_modules_width = frozenset(
        {
        }
    )
    lmc_modules_widthback = frozenset(
        {
            TriMFBlock,
            TriMFBlocksub1,
            TriMFBlocksub2,
    ...
    lmc_modules_widthback2 = frozenset(
        {
            BiMFBlock,
        }
    )
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        n = n_ = max(round(n * depth), 1) if n > 1 else n  # depth gain  # 这一步，让2失效了
        if m in base_modules:
            ...
            c1 = ch[f]
            args = [*args[1:]]
        elif m in lmc_modules:
            c2 = args[1]
        elif m in lmc_modules_width:
            args[0] = ch[f[0]] # 只用第一个做输入
            c2 = args[0]
        elif m in lmc_modules_widthback:
            args[0] = (ch[f[0]], ch[f[1]], ch[f[2]]) # 三个做输入
            c2 = ch[f[0]]
        elif m in lmc_modules_widthback2:
            args[0] = (ch[f[0]], ch[f[1]]) # 三个做输入
            c2 = ch[f[0]]
